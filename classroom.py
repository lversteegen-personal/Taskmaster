...
class classroom:
    ...
    def run_training_batch(self, n_problems, epochs_per_episode):
        ...
        for rd in replay_record:
            ...
            rewards.append(rd.eval_tree_root.q)

            end_node: task_tree_node = proof_nodes[rd.task_index]
            values.append(self.task.reward_function(
                end_node.state)*self.student.reward_discount**(end_node.depth-rd.task_node.depth))

            reward_confidences.append(np.maximum(
                0.001, np.abs(rd.task_node.initial_reward-rd.eval_tree_root.q)))

            reward_weights.append(rd.eval_tree_root.n/rd.eval_tree_root.n.sum())
        ...
        if self.input_buffer is None:
            ...
            self.reward_weight_buffer = reward_weights
        else:
            ...
            self.input_buffer = np.concatenate(
                [inputs, self.input_buffer[:end]])
            self.value_buffer = np.concatenate(
                [values, self.value_buffer[:end]])
            self.reward_buffer = np.concatenate(
                [rewards, self.reward_buffer[:end]])
            self.reward_confidence_buffer = np.concatenate(
                [reward_confidences, self.reward_confidence_buffer[:end]])
            self.reward_weight_buffer = np.concatenate(
                [reward_weights, self.reward_weight_buffer[:end]])

        state_inputs = self.task.make_input(self.input_buffer)

        self.student.neural_network.fit_value(
            state_inputs, self.value_buffer, self.reward_buffer, self.reward_confidence_buffer, self.reward_weight_buffer, epochs_per_episode)
    ...
    def test_students(self, start_states):
        ...
        for s in range(self.max_steps):
            ...
            unfinished_task_indices = [
                i for i, p in enumerate(task_nodes) if not p.completed]
            if len(unfinished_task_indices) == 0:
                break

            print(
                f"Before step {s+1}, {len(unfinished_task_indices)} out of {len(task_nodes)} remain open.")
            result = self.student.run_action_step(
                [task_nodes[i] for i in unfinished_task_indices])
            ...
            for j, (action, pi, eval_root) in enumerate(result):
                ...
                datum = replay_datum(old_node, new_node,
                                     action, pi, eval_root, k)
                replay_record.append(datum)

        unfinished_task_indices = [
            i for i, p in enumerate(task_nodes) if not p.completed]
        print(
            f"After step {s+1}, {len(unfinished_task_indices)} out of {len(task_nodes)} remain open.")
